...
class LoadDatabase {
	...
	@Bean
	CommandLineRunner initDatabase(UserRepository userRepository, MatiereRepository matiereRepository,
			NoteTypeRepository noteTypeRepository, EvaluationRepository evaluationRepository, // Ajout
			NoteRepository noteRepository, PasswordEncoder passwordEncoder) { // Ajout
		...
		return args -> {
			...
			if (matiereRepository.count() == 0) {
				...
				// (Ceci est une simplification pour l'exemple, en prod on ferait des
				// findByName)
			...
			if (noteTypeRepository.count() == 0) {
				...
				if (!allTypes.isEmpty()) {
					ds = allTypes.stream().filter(t -> t.getLibelle().equals("Devoir Surveillé")).findFirst()
							.orElse(null);
			...
			User profMath = new User("Bernard", "Sophie", Role.ENSEIGNANT, "sophie.bernard@example.com",
					"Boulevard de la Gare", "0611223344", defaultPassword);
			User profCyber = new User("Enseignant", "Durand", Role.ENSEIGNANT, "pierre.durand@example.com",
					"Avenue Victor Hugo", "0605060708", defaultPassword);
			User profAnglais = new User("Petit", "Claire", Role.ENSEIGNANT, "claire.petit@example.com", "Rue Nationale",
					"0622334455", defaultPassword);
			...
			if (userRepository.count() == 0) {
				...
				userRepository.save(new User("Guerin", "Alice", Role.ENSEIGNANT, "alice.guerin@example.com",
						"Rue du Parc", "0612121212", defaultPassword));
				userRepository.save(new User("Test", "prof", Role.ENSEIGNANT, "prof@lycee.local", "test", "0102030405",
						defaultPassword));

				// Sauvegarde des admins
				userRepository.save(new User("Admin", "Martin", Role.ADMIN, "martin.admin@example.com",
						"Rue de la Liberté", "0708091011", defaultPassword));
				userRepository.save(new User("Renard", "Maxime", Role.ADMIN, "maxime.renard@example.com",
						"Impasse des Pins", "0688997788", defaultPassword));
				...
				etudiants.add(userRepository.save(new User("Leclerc", "Emma", Role.ETUDIANT, "emma.leclerc@example.com",
						"Rue des Acacias", "0602030405")));
				etudiants.add(userRepository.save(new User("Morel", "Lucas", Role.ETUDIANT, "lucas.morel@example.com",
						"Rue des Érables", "0677889900")));
				etudiants.add(userRepository.save(new User("Dupont", "Nicolas", Role.ETUDIANT,
						"nicolas.dupont@example.com", "Rue de la Mairie", "0644556677")));
				etudiants.add(userRepository.save(
						new User("Test", "etudiant", Role.ETUDIANT, "etudiant@lycee.local", "test", "0102030405")));

				log.info("Utilisateurs chargés.");
			}

			// --- 4. Création des EVALUATIONS et NOTES ---
			// On ne crée les données que si les tables sont vides pour éviter les doublons
			// au redémarrage
